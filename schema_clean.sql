-- ─────────────────────────────────────────────────────────────────────────────
-- LIFE ARCHITECT APP — Clean slate schema
-- Safe to run even if you tried before. Drops everything and rebuilds fresh.
...
-- Daily entries: moon rating, journal, evening check-in
...
-- Habit logs: Life Architect + Skinni checkboxes
...
-- Floor logs: daily non-negotiables
...
-- Time entries: timer sessions and manual entries
...
-- Weekly reviews: Life Architect + Skinni
...
CREATE POLICY "own_daily_entries"   ON daily_entries  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "own_habit_logs"      ON habit_logs     FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "own_floor_logs"      ON floor_logs     FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "own_time_entries"    ON time_entries   FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "own_weekly_reviews"  ON weekly_reviews FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ─────────────────────────────────────────────────────────────────────────────
-- Done. Five tables created and secured.
-- ─────────────────────────────────────────────────────────────────────────────
